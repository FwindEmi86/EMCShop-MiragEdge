...
public class DarkMatter {
    ...
    public DarkMatter(EMCShopMiragEdge plugin) {
        this.plugin = plugin;
        ConfigStrings c = plugin.getConfigMainClass().getStrings();

        // 使用兼容方法创建暗物质头颅
        item = createCustomSkull(SkullTextures.ITEM_DARK_MATTER);
    ...
    private ItemStack createCustomSkull(String base64) {
        ...
        try {
            ...
            // 最终回退方案
            plugin.getLogger().log(java.util.logging.Level.WARNING, "设置暗物质头颅时出错，使用备用方法", e);
            try {
                meta.setOwningPlayer(plugin.getServer().getOfflinePlayer("FwindEmi"));
            } catch (Exception ex) {
